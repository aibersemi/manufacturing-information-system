...
import {
	useBackendApiMasterdataCreateCustomer,
	useBackendApiMasterdataDeleteCustomer,
	useBackendApiMasterdataListCustomers,
	useBackendApiMasterdataUpdateCustomer,
} from "@/api/generated/master-data/master-data";
import type { CustomerResponse } from "@/api/generated/models/customerResponse";
...
export function CustomersRouteScreen() {
	return (
		<AccessGuard anyOf={["masterdata.customers.read"]}>
			<CustomersScreen />
		</AccessGuard>
	);
}
...
const customerSchema = z.object({
	name: z.string().min(1, m.customers_name_required()),
	phone: z.string(),
	address: z.string(),
...
function CustomersScreen() {
	const search = useSearch({ from: customersRoute.id });
	const navigate = useNavigate({ from: customersRoute.id });
	const uiStore = useMemo(
		() =>
			new Store({
				selected: null as CustomerResponse | null,
				deleteTarget: null as CustomerResponse | null,
			}),
		[],
	);
	...
	const canCreate = can(capabilityList, "masterdata.customers.create");
	const canUpdate = can(capabilityList, "masterdata.customers.update");
	const canDelete = can(capabilityList, "masterdata.customers.delete");
	const { data: response, isLoading, refetch } = useBackendApiMasterdataListCustomers();
	const customers = response?.data;
	const createMutation = useBackendApiMasterdataCreateCustomer();
	const updateMutation = useBackendApiMasterdataUpdateCustomer();
	const deleteMutation = useBackendApiMasterdataDeleteCustomer();
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			try {
				...
				if (selected) {
					await updateMutation.mutateAsync({ customerId: selected.id, data: payload });
				} else {
	...
	const customerColumns = useMemo<ColumnDef<CustomerResponse>[]>(
		() => [
			{
				accessorKey: "name",
	...
	const confirmDelete = async () => {
		...
		try {
			await deleteMutation.mutateAsync({ customerId: deleteTarget.id });
			toast.success(m.common_mutation_success());
			setUi({
		...
		<div className="flex flex-col gap-6 p-6 lg:p-8">
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div className="flex flex-col gap-1">
					<h1 className="text-2xl font-bold">{m.customers_title()}</h1>
					<p className="text-sm text-muted-foreground">{m.customers_description()}</p>
				</div>
				{canCreate || canUpdate ? (
					<Sheet
						open={isFormOpen}
					...
					>
						...
						<SheetContent className="overflow-y-auto">
							<SheetHeader>
								<SheetTitle>{selected ? m.common_edit() : m.customers_add()}</SheetTitle>
								<SheetDescription>
									{selected ? m.customers_edit_description() : m.customers_add_description()}
								</SheetDescription>
							</SheetHeader>
							<form
								onSubmit={(event) => {
								...
								<FieldGroup className="gap-4">
									...
									<form.Field name="phone">
										{(field) => {
											...
											return (
												<Field data-invalid={invalid}>
													<FieldLabel htmlFor="customer-phone">{m.common_phone()}</FieldLabel>
													<Input
														id="customer-phone"
														name={field.name}
														value={field.state.value}
									...
									<form.Field name="address">
										{(field) => {
											...
											return (
												<Field data-invalid={invalid}>
													<FieldLabel htmlFor="customer-address">{m.common_address()}</FieldLabel>
													<Textarea
														id="customer-address"
														name={field.name}
														value={field.state.value}
									...
									<form.Field name="status">
										{(field) => (
											<Field>
												<FieldLabel htmlFor="customer-status">{m.common_status()}</FieldLabel>
												<NativeSelect
													id="customer-status"
													value={field.state.value}
													onChange={(event) =>
			...
			<Card>
				<CardHeader>
					<CardTitle>{m.customers_list_title()}</CardTitle>
				</CardHeader>
				<CardContent>
					{isLoading ? (
						<DataLoading />
						...
						<DataEmpty
							title={m.customers_empty_title()}
							description={m.customers_empty_description()}
						/>
					)}
				</CardContent>
			</Card>
